...
class PanListener$Impl extends __lib.NativeBase implements PanListener {
  ...
  onPan(GestureState state, Point2D origin, Point2D translation, double velocity) {
    ...
    final __resultHandle = _onPanFfi(_handle, __lib.LibraryContext.isolateId, _stateHandle, _originHandle, _translationHandle, _velocityHandle);
...
int _sdkGesturesPanlisteneronPanStatic(int _token, int state, Pointer<Void> origin, Pointer<Void> translation, double velocity) {

  try {
    (__lib.instanceCache[_token] as PanListener).onPan(sdkGesturesGesturestateFromFfi(state), sdkCorePoint2dFromFfi(origin), sdkCorePoint2dFromFfi(translation), (velocity));
  } finally {
...
Pointer<Void> sdkGesturesPanlistenerToFfi(PanListener value) {
  ...
  final result = _sdkGesturesPanlistenerCreateProxy(
    ...
    Pointer.fromFunction<Uint8 Function(Uint64, Uint32, Pointer<Void>, Pointer<Void>, Double)>(_sdkGesturesPanlisteneronPanStatic, __lib.unknownError)
  );
