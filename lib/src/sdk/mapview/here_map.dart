...
class _HereMapState extends State<HereMap> implements MapCameraObserver {
  ...
  _MapOrientationChangeHandler _orientationHandler =
      _MapOrientationChangeHandler();
  ...
  void initState() {
    ...
    // });
   // LibraryContext.init(IsolateOrigin.main,nativeLibraryPath: "here_sdk");
    super.initState();
  }
  ...
  List<Widget>? _buildPins(BuildContext context, List<WidgetPin> pins) {
    List<Widget>? pinWidgets =
        pins.map((WidgetPin pin) => pin.makeWidget(context)).toList();
  ...
  _updatePins() {
    final List<WidgetPin> newPins =
        _mapController!.widgetPins.map((WidgetPin pin) {
      final Point2D point =
          _mapController!.geoToViewCoordinates(pin.coordinates);
...
class _MapOrientationChangeHandler {
  ...
  handleOrientationChange(
      BuildContext context, Function orientationChangeCallback) {
