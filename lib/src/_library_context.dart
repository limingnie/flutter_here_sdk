...
final nativeLibrary =
    _nativeLibrary ?? _loadNativeLibrary(_getLibraryPath("here_sdk"));
/*late*/
...
final _libraryCallbacksQueueInit = catchArgumentError(() =>
    nativeLibrary.lookupFunction<Int32 Function(Uint8), int Function(int)>(
        'here_sdk_library_callbacks_queue_init'));
final _libraryCallbacksQueueRelease = catchArgumentError(() =>
    nativeLibrary.lookupFunction<Void Function(Int32), void Function(int)>(
        'here_sdk_library_callbacks_queue_release'));
final _libraryWaitForCallbacks = catchArgumentError(() =>
    nativeLibrary.lookupFunction<Uint8 Function(Int32), int Function(int)>(
        'here_sdk_library_wait_for_callbacks'));
final _libraryExecuteCallbacks = catchArgumentError(() =>
    nativeLibrary.lookupFunction<Void Function(Int32), void Function(int)>(
...
enum IsolateOrigin { main, spawned }

enum WaitCallbackResult { stopped, hasIncoming, timedOut }
...
class LibraryContext {
  ...
  /*late*/
  static StreamSubscription? _callbackStream;
  ...
  static void init(IsolateOrigin isolateOrigin, {String? nativeLibraryPath}) {
    ...
    _isolateId =
        _libraryCallbacksQueueInit(isolateOrigin == IsolateOrigin.main ? 1 : 0);

    final receivePort = ReceivePort();
    Isolate.spawn(
        _sentryIsolate,
        _SentryIsolateMessage(
            receivePort.sendPort, isolateId, nativeLibraryPath!));
    _callbackStream = receivePort.listen((dynamic _) {
  ...
  static void _sentryIsolate(_SentryIsolateMessage message) {
    ...
    do {
      waitResult = WaitCallbackResult
          .values[_libraryWaitForCallbacks(message.isolateId)];
  ...
  static void _loadCustomLibrary(String? nativeLibraryPath) {
    if (nativeLibraryPath != null)
      _nativeLibrary = _loadNativeLibrary(nativeLibraryPath);
  }
}
