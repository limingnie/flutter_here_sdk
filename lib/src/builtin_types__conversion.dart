...
import 'package:ffi/src/arena.dart';
...
String stringFromFfi(Pointer<Void> handle) {
  var pointer =  _stringGetValue(handle) as Pointer<Utf8>;
  return pointer.toDartString();
}

void stringReleaseFfiHandle(Pointer<Void> handle) {
  return _stringReleaseHandle(handle);
}
...
Pointer<Void> localeToFfi(Locale locale) {
  ...
  final cCountryCode =
    locale.countryCode != null ? locale.countryCode.toString().toNativeUtf8() : Pointer<Utf8>.fromAddress(0);
  final cScriptCode =
    locale.scriptCode != null ? locale.scriptCode.toString().toNativeUtf8() : Pointer<Utf8>.fromAddress(0);
  ...
  //
  calloc.free(cLanguageTag);
   //free(cLanguageCode);
  //Arena().releaseAll();
  if (cCountryCode.address != 0) calloc.free(cCountryCode); //free(cCountryCode);
  if (cScriptCode.address != 0)  calloc.free(cScriptCode);//free(cScriptCode);
  //Arena().releaseAll();
  // free(cLanguageTag);
  calloc.free(cLanguageTag);
  return result;
...
Locale localeFromFfi(Pointer<Void> handle) {
  ...
  return Locale.fromSubtags(
    languageCode: languageCodeCstring.address != 0 ? languageCodeCstring.toDartString() : "",
    countryCode: countryCodeCstring.address != 0 ? countryCodeCstring.toDartString() : "",
    scriptCode: scriptCodeCstring.address != 0 ? scriptCodeCstring.toDartString() : ""
  );
}
...
Uint8List blobFromFfiNullable(Pointer<Void> handle) {
  if (handle.address == 0) return Uint8List.fromList([]);
